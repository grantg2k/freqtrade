...
class Alpaca(Exchange):
    ...
    def fetch_open_orders(self, pair: Optional[str] = None) -> Iterable[dict[str, Any]]:
        orders = self.loop.run_until_complete(
            self._async_call(self._rest.list_orders, status="open", symbols=[self._to_symbol(pair)] if pair else None)
        )
        return [o._raw for o in orders]

    def fetch_closed_orders(self, pair: Optional[str] = None) -> Iterable[dict[str, Any]]:
        orders = self.loop.run_until_complete(
            self._async_call(self._rest.list_orders, status="closed", symbols=[self._to_symbol(pair)] if pair else None)
        )
        return [o._raw for o in orders]
