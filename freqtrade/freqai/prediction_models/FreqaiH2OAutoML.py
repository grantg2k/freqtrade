...
# h2o.init(max_mem_size="200G", bind_to_localhost=False)
...
class FreqaiH2OAutoML(IFreqaiModel):
    ...
    def fit(self, data_dictionary: Dict, dk: FreqaiDataKitchen, **kwargs) -> Any:
        ...
        fold_size = len(validation_labels_df) // nfolds
        fold_col = np.repeat(np.arange(nfolds), fold_size)
        validation_labels_df['fold_numbers'] = np.append(fold_col, 
                                                    np.arange(len(validation_labels_df) % nfolds))
        ...
        model.train(
            ...
            # weights_column = train_weights_df.columns.tolist()[0]
        )
...
#             logger.info(classification_report(eval_set[1], preds['predict']))
        ...
        elif self.freqai_info.get('data_split_parameters', {}).get('test_size', 0.1) != 0 and self.regression: #regression report
            # ensemble = h2o.estimators.stackedensemble.H2OStackedEnsembleEstimator(
            test_features = H2OFrame(pd.DataFrame(data_dictionary["test_features"]))
            preds=model.leader.predict(test_features).as_data_frame()
            eval_set = (pd.DataFrame(data_dictionary["test_features"]),
                        pd.DataFrame(data_dictionary["test_labels"].to_numpy()[:, 0]))
            plt.figure(figsize=(12, 6))
            plt.plot(eval_set[1], label='Actual')
            plt.plot(preds, label='Predictions')
            plt.legend()
            plt.savefig(f'actual_vs_predictions_{datetime.now()}.png')
            plt.show()
    ...
    def define_label_pipeline(self, threads=-1) -> Pipeline:
        ft_params = self.freqai_info["feature_parameters"]
        label_pipeline_steps = [
            ('scaler', SKLearnWrapper(MinMaxScaler(feature_range=(-1, 1))))
            ]
        
        sigma = ft_params.get('label_noise_standard_deviation', 0)
        
        if sigma:
            label_pipeline_steps.append(('noise', ds.Noise(sigma=sigma)))
        
        label_pipeline = Pipeline(label_pipeline_steps)
    ...
    def train(
        ...
        if self.regression:
            ...
            logger.info("Regression detected, scaling features")
        else:
            logger.info("Classification detected, features will not be scaled")
